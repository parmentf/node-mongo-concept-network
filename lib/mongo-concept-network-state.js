    ...
    this.cn.db.eval(function () { // eval on mongo server
      ...
      .forEach(function (node) {
        ...
        .forEach(function (link) {
          ...
          infl += 0.5 + nodeState[node._id].oldActivationValue * link.coOcc;
      ...
      .snapshot()
      .forEach(function (node) {
        var nodeState = nodeState[node._id] !== undefined ? 
                          nodeState[node._id] : 
                        { activationValue: 0, oldActivationValue: 0, age: 0 };
        var decay = 40;
        var memoryPerf = 100;
        var minusAge = 200 / ( 1 + Math.exp(-nodeState.age / memoryPerf)) - 100;
        var newActivationValue;
        // If this node is not influenced at all
        if (influenceValue[nodeId] === undefined || !influenceValue[nodeId]) {
        ...
        nodeState.activationValue = newActivationValue;
        if(newActivationValue) {
          db.conceptnetwork.save(nodeState);
        }
        else {
